...
def build_eml_text(item: dict) -> str:
    ...
    def _clean_header_value(v: object) -> str:
        s = str(v)
        s = s.replace("\r\n", " ").replace("\n", " ").replace("\r", " ")
    ...
    for key, values in headers.items():
        if isinstance(values, list):
            for value in values:
                header_lines.append(f"{key}: {_clean_header_value(value)}")
        else:
            header_lines.append(f"{key}: {_clean_header_value(values)}")

    if header_lines:
        return "\r\n".join(header_lines) + "\r\n\r\n" + body

    raw_data = item.get("Raw", {}).get("Data", "") or ""
    return str(raw_data)
...
def main():
    ...
    with args.metadata_csv.open("a", encoding="utf-8", newline="") as handle:
        ...
        for item in items:
            ...
            eml_text = build_eml_text(item)
            if not eml_text.strip():
                continue
            ...
            with eml_path.open(
                "w", encoding="utf-8", errors="ignore", newline=""
            ) as out:
                out.write(eml_text)
