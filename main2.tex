...
Druhou vetvou bol campaign-specialized variant, ktorý kombinuje ne-leaky campaign-style adaptačný tréning, zúženie spam triedy a OVR subclass hlavu. Tento variant sa ukázal ako najsilnejší na špecializovaných kampaniach z Experimentov 2 a 3:
\begin{itemize}
    \item \textbf{Špecializovaný campaign model:} \\
    \textit{scenario\_m\_anti\_template\_feature\_regularized + hybrid\_logreg + text\_plus\_features + adaptation + spam narrowing + OVR}
\end{itemize}
...
\begin{tabular}{lccc}
\toprule
\textbf{Dataset} & \textbf{L+SGD} & \textbf{M baseline} & \textbf{Tuned single-stage} \\
\midrule
test\_hard\_source   & 0.694 & 0.802 & 0.874 \\
test\_hard\_cluster  & 0.820 & 0.848 & 0.864 \\
test\_deployment     & 0.876 & 0.896 & 0.908 \\
\bottomrule
\end{tabular}
...
a SHAP-guided tuned single-stage konfigurácia \textit{M + hybrid\_logreg}
dosahuje najlepší celkový kompromis na náročných testoch.
...
Ako finálny general/offline model bol zvolený variant:
\texttt{scenario\_m\_anti\_template\_feature\_regularized + hybrid\_logreg + text\_plus\_features}
so SHAP-guided feature subsetom a tuned class scalingom.
...
Trieda \texttt{legit} je klasifikovaná veľmi spoľahlivo, čo dokumentuje vysoký počet správne klasifikovaných vzoriek (14916) a relatívne nízky počet chýb smerom k ostatným triedam (najmä 141 do \texttt{phishing} a 187 do \texttt{spam}). To naznačuje, že model dokáže robustne odlišovať bežnú komunikáciu od škodlivého obsahu.

Trieda \texttt{phishing} dosahuje taktiež veľmi dobré výsledky (2616 správne klasifikovaných vzoriek), pričom chyby sú rozdelené medzi \texttt{spam} (46 prípadov) a \texttt{legit} (64 prípadov). Tento vzor chýb odráža prirodzené prekrytie medzi manipulatívnym marketingovým obsahom a phishingovými technikami.

Trieda \texttt{financial\_fraud} je klasifikovaná stabilne (332 správnych klasifikácií), pričom väčšina chýb smeruje do triedy \texttt{spam} (20 prípadov). Tento jav je očakávaný, keďže finančné podvody často využívajú podobné jazykové vzory ako agresívny marketing alebo spam.

Najproblematickejšou triedou ostáva \texttt{spam}. Aj keď model správne klasifikoval 799 vzoriek, dochádza k zámene s \texttt{legit} (21 prípadov) a \texttt{phishing} (9 prípadov). To naznačuje, že hranica medzi spamom a ostatnými kategóriami je v reálnych dátach menej jednoznačná a výrazne závisí od kontextu správy.
...
Z pohľadu metodickej čistoty aj praktického použitia sa tento variant javí ako najvhodnejší finálny general/offline model. Campaign-specialized adapted+OVR ostáva dôležitým doplnkovým variantom pre špecializované benchmarky, no baseline zo scenára L naďalej slúži ako porovnávacia anti-source konfigurácia.
...
\begin{figure}[H]
    ...
    \caption{Porovnanie heuristického prístupu, tuned single-stage modelu a campaign-specialized variantu adapted+OVR na kampani z Experimentu 1.}
    \label{fig:exp1_model_comparison}
\end{figure}

Z grafu je zrejmé, že heuristický analyzátor dosahuje vysoký recall, avšak výrazne zaostáva v metrikách balanced accuracy a MCC. Tuned single-stage model a variant adapted+OVR obe poskytujú stabilnejšiu separáciu tried a lepší kompromis medzi precision a recall. Variant adapted+OVR dosahuje na zmiešanej kampani mierne vyššie binárne F1, no jeho zisk sa nepretavuje do lepšieho multiclass výkonu.
...
\begin{figure}[H]
    ...
    \caption{Porovnanie multiclass metrík heuristiky, tuned single-stage modelu a variantu adapted+OVR na dátach Experimentu 1.}
    \label{fig:exp1_multiclass_comparison}
\end{figure}

Výsledky ukazujú, že samotná heuristika dosahuje výrazne nižší multiclass výkon než modelové prístupy. Na zmiešanom sender-clean benchmarku ostáva najsilnejším multiclass variantom tuned single-stage model zo scenára M (\texttt{mc\_f1\_macro = 0.6213}), zatiaľ čo adapted+OVR dosahuje porovnateľné, ale mierne nižšie skóre (\texttt{0.6198}). To naznačuje, že campaign-style adaptácia pomáha najmä na úzko špecializovaných benchmarkoch, nie na obsahovo zmiešanej kampani Experimentu 1.
...
\begin{figure}[H]
    ...
    \caption{Konfúzna matica tuned single-stage modelu na dátach Experimentu 1.}
    \label{fig:exp1_confusion_matrix}
\end{figure}

Z konfúznej matice je možné pozorovať, že tuned single-stage model veľmi spoľahlivo klasifikuje triedy \texttt{legit}, \texttt{phishing} a \texttt{financial\_fraud}, pričom najväčšie problémy vznikajú pri triede \texttt{spam}. Tá zostáva najčastejšie zamieňaná s \texttt{phishing}, čo je konzistentné aj s ostatnými experimentmi.
...
Z pohľadu výsledkov sa ako najsilnejší \textbf{generalistický} variant pre Experiment 1 ukázal retunovaný model \texttt{hybrid\_logreg} so vstupom \texttt{text\_plus\_features}. Pri finálnom kampanovom vyhodnotení s neutrálnymi class scales dosiahol na 250 doručených správach \texttt{bin\_f1 = 0.9727} a \texttt{mc\_f1\_macro = 0.6213}. Oproti staršiemu runu to predstavuje zlepšenie o \texttt{+0.0818} v binárnom F1 a o \texttt{+0.5027} v multiclass macro F1.

Dodatočne bol testovaný aj campaign-specialized variant s campaign-style adaptačným tréningom, zúženou spam triedou a \textit{one-vs-rest} subclass hlavou. Tento variant zvýšil binárne F1 na \texttt{0.9778}, ale multiclass macro F1 mierne kleslo na \texttt{0.6198}. Na zmiešanom benchmarku Experimentu 1 sa preto ako vhodnejšia voľba ukázal tuned single-stage model, zatiaľ čo adapted+OVR je výhodnejší najmä pre špecializované kampane z Experimentov 2 a 3.
...
\texttt{ml\_only}               & \textbf{0.97} & \textbf{0.9813} & \textbf{1.0000} & 0.9625 & \textbf{0.9809} \\
\texttt{heuristic\_plus\_ml}    & 0.92 & 0.8188 & 0.9186 & 0.9875 & 0.9518 \\
\bottomrule
\end{tabular}
\end{table}
...
Najlepší výkon dosiahol prístup \texttt{ml\_only}, ktorý v tomto experimente používa campaign-style adaptačný tréning, zúženie spam triedy a \textit{one-vs-rest} subclass hlavu. Tento variant poskytuje výrazne lepší kompromis medzi precision a recall a zároveň dosahuje najvyššiu hodnotu F1 skóre (\texttt{0.9809}). Kombinovaný prístup \texttt{heuristic\_plus\_ml} ostáva silný, no už nedosahuje výkon samostatného adaptovaného modelu.

Z doplnkového pohľadu boli analyzované aj multiclass výsledky, ktoré však nie sú primárnym cieľom tohto experimentu. V tomto režime heuristika dosahuje nízke hodnoty (napr. \texttt{mc\_f1\_macro = 0.13}), čo je očakávané, keďže nie je navrhnutá na jemné rozlišovanie medzi triedami \texttt{phishing}, \texttt{spam} a \texttt{financial\_fraud}. Adaptovaný model strojového učenia dosahuje v multiclass nastavení výrazne lepší výsledok než pôvodný tuned single-stage variant (\texttt{mc\_f1\_macro = 0.5865} vs. \texttt{0.3262}). To ukazuje, že pri header/auth kampani bol hlavný problém v nesúlade medzi tréningovou distribúciou a campaign-style vstupmi, nie v samotnej kapacite modelu.
...
\begin{itemize}
    ...
    \item zúženie spam triedy a \textit{one-vs-rest} subclass hlava znižujú kolaps tried \texttt{phishing} a \texttt{financial\_fraud} do \texttt{spam}.
\end{itemize}
...
Heuristický analyzátor spoľahlivo deteguje explicitné bezpečnostné indikátory a funguje ako prvá vrstva rozhodovania (legit vs suspicious). Najlepšie výsledky však v Track B dosiahol campaign-specialized model s adaptačným tréningom, zúžením spam triedy a \textit{one-vs-rest} subclass hlavou, ktorý dosiahol \texttt{bin\_f1 = 0.9809} a \texttt{mc\_f1\_macro = 0.5865}.
...
ml\_only & 0.94 & 1.0000 & 0.9250 & 0.9610 \\
heuristic\_plus\_ml & 0.93 & 0.9195 & 1.00 & 0.9581 \\
\hline
\end{tabular}
\end{table}
...
Po zavedení campaign-style adaptačného tréningu, zúženia spam triedy a \textit{one-vs-rest} subclass hlavy dosiahol samotný model strojového učenia na URL-heavy kampani najlepší binárny výkon (\texttt{F1 = 0.9610}). Kombinovaný prístup \texttt{heuristic\_plus\_ml} zostáva veľmi silný (\texttt{0.9581}), ale už neprináša zlepšenie nad adaptovaný ML model.
...
Experiment 3 ukázal, že URL-heavy benchmark výrazne profituje z priblíženia tréningovej distribúcie ku campaign-style vstupom. Adaptovaný model s OVR subclass hlavou dokázal výrazne zlepšiť nielen binárny výkon, ale aj jemné multiclass rozlíšenie medzi \texttt{phishing}, \texttt{spam} a \texttt{financial\_fraud}. URL heuristiky preto ostávajú cenným doplnkovým signálom, no ich najlepší efekt sa dosiahol až po špecializácii tréningu a rozhodovacieho headu modelu.
...
\caption{Porovnanie tuned single-stage a adapted+OVR variantu na hlavných offline splitoch v Experimente 4 (Track A). Graf ukazuje, že adapted+OVR pomáha na špecializovaných campaign benchmarkoch, ale na shared offline splitoch zaostáva za tuned single-stage modelom.}
\label{fig:exp4_trackA_deployment}
\end{figure}

Výsledky ukazujú, že tuned single-stage model ostáva na shared offline splitoch jednoznačne silnejší než adapted+OVR. Variant \texttt{scenario\_m\_anti\_template\_feature\_regularized + hybrid\_logreg} dosiahol na \texttt{test\_deployment} \texttt{macro F1 = 0.9076} a súčasne výrazne zlepšil výsledok na \texttt{test\_hard\_source} na \texttt{0.8744}. Dodatočne testovaný campaign-specialized variant adapted+OVR síce výrazne pomohol na Experimentoch 2 a 3, no na offline splitoch klesol na \texttt{0.8488} (\texttt{test\_iid}), \texttt{0.7538} (\texttt{test\_hard\_source}), \texttt{0.7966} (\texttt{test\_hard\_cluster}) a \texttt{0.8374} (\texttt{test\_deployment}).
...
a \texttt{mc\_f1\_macro = 0.6213}. Campaign-specialized adapted+OVR
variant dosiahol mierne vyššie binárne F1 (\texttt{0.9778}), ale
multiclass výkon sa nezlepšil (\texttt{0.6198}). Táto časť preto
slúži ako praktické overenie, že Exp1 ostáva najlepšie reprezentovaný
generalistickým tuned modelom, zatiaľ čo adapted+OVR je vhodnejší
pre úzko špecializované kampane.
...
Na kampani z Experimentu~2 \ref{epx2} dosiahol campaign-specialized
variant adapted+OVR hodnoty
\begin{itemize}
    \item \texttt{mc\_accuracy = 0.58},
    \item \texttt{mc\_balanced\_accuracy = 0.6404},
    \item \texttt{mc\_f1\_macro = 0.5865}.
\end{itemize}

Na kampani z Experimentu~3 \ref{epx3} dosiahol
\begin{itemize}
    \item \texttt{mc\_accuracy = 0.60},
    \item \texttt{mc\_balanced\_accuracy = 0.6829},
    \item \texttt{mc\_f1\_macro = 0.6052}.
\end{itemize}


Pre kontext je vhodné uviesť výkon v Experimente~1\ref{epx1},
kde tuned single-stage model dosiahol \texttt{mc\_f1\_macro = 0.6213}.
Adapted+OVR variant dosiahol na tej istej kampani \texttt{0.6198},
čo potvrdzuje, že jeho prínos je viazaný najmä na špecializované
header/auth a URL benchmarky, nie na obsahovo zmiešaný scenár.

V porovnaní s binárnym hodnotením ostáva multiclass úloha náročnejšia,
avšak adapted+OVR výrazne znížil pôvodný kolaps tried
\texttt{phishing} a \texttt{financial\_fraud} do \texttt{spam}.
Výsledky tak ukazujú, že špecializácia tréningu a rozhodovacieho
headu môže výrazne zlepšiť subclass diskrimináciu, no za cenu
...
Na základe kombinácie výsledkov zo všetkých splitov a kampaní sa ukázalo,
že neexistuje jeden univerzálne najsilnejší variant pre všetky typy dát.
Ako \textbf{finálny general/offline model} bol preto zvolený retunovaný
variant \texttt{scenario\_m\_anti\_template\_feature\_regularized}
s modelom \texttt{hybrid\_logreg}. V SHAP-guided nastavení dosiahol
\texttt{f1\_macro = 0.9105} na \texttt{test\_iid},
\texttt{0.8744} na \texttt{test\_hard\_source},
\texttt{0.8638} na \texttt{test\_hard\_cluster} a
\texttt{0.9076} na \texttt{test\_deployment}.

Ako \textbf{finálny campaign-specialized variant} pre špecializované
header/auth a URL kampane bol zvolený model s campaign-style adaptačným
tréningom bez leakage, zúženou spam triedou a \textit{one-vs-rest}
subclass hlavou (adapted+OVR). Tento variant dosiahol na Exp2
\texttt{bin\_f1 = 0.9809}, \texttt{mc\_f1\_macro = 0.5865} a na Exp3
\texttt{bin\_f1 = 0.9610}, \texttt{mc\_f1\_macro = 0.6052}, ale jeho
offline výsledky boli citeľne slabšie než pri tuned single-stage modeli.
...
scenára, rozhodovacieho headu a dátovej špecializácie je kľúčová
pre dosiahnutie optimálneho výkonu. Tuned single-stage model predstavuje
robustné riešenie pre všeobecné nasadenie a offline generalizáciu,
zatiaľ čo adapted+OVR je vhodný ako špecializovaný variant pre
campaign benchmarky s výrazným header/auth alebo URL biasom.
...
Ako reprezentant modelového prístupu bol použitý tuned single-stage model zo scenára
...
najlepší kompromis medzi offline robustnosťou, multiclass výkonom a generalizáciou.
...
detekčného systému, ktorá oddeľuje všeobecný robustný model od
špecializovaného campaign-oriented variantu. Toto rozdelenie sa ukázalo
ako metodicky čistejšie než snaha vynútiť jeden univerzálny model pre
všetky typy dát.

\paragraph{Zvolená konfigurácia}

\begin{itemize}
    \item \textbf{Primárny general/offline model:} \texttt{scenario\_m\_anti\_template\_feature\_regularized + hybrid\_logreg} v režime \texttt{text\_plus\_features} so SHAP-guided feature subsetom.
    \item \textbf{Špecializovaný campaign variant:} campaign-style adaptačný tréning bez leakage, zúženie spam triedy a \textit{one-vs-rest} subclass hlava nad rovnakou textovo-feature reprezentáciou.
    ...
    \begin{itemize}
        \item pre všeobecné nasadenie a offline generalizáciu je preferovaný tuned single-stage model,
        \item pre špecializované header/auth a URL-heavy campaign scenáre je vhodný adapted+OVR variant.
    \end{itemize}
...
\begin{itemize}
    ...
    no najväčšie zlepšenie priniesol až campaign-style adaptačný tréning,
    zúženie spam triedy a OVR subclass hlava.
