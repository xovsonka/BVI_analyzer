...
Na tento účel bol implementovaný prípravný pipeline, ktorého úlohou bolo načítať vstupné CSV súbory z rôznych datasetov, vyčistiť textové polia, odstrániť neplatné a duplicitné záznamy a zjednotiť výsledok do spoločnej schémy. Dôležitou metodickou vlastnosťou finálnej verzie pipeline bolo oddelenie \textbf{modelového textu} a \textbf{surových polí}. Modelový text (
\texttt{subject}, \texttt{body}, \texttt{text}) bol určený pre textovú reprezentáciu modelu, zatiaľ čo surové polia (
\texttt{subject\_raw}, \texttt{body\_raw}, \texttt{text\_raw} a surové hlavičky) zostali zachované pre audit a extrakciu technických indikátorov.

Tento dvojitý zápis sa ukázal ako dôležitý nielen z hľadiska transparentnosti, ale aj z hľadiska korektnej extrakcie príznakov. V neskoršej iterácii experimentov sa totiž ukázalo, že časť nových URL a header feature mala v niektorých tréningových scenároch takmer nulové pokrytie, hoci v zdieľanom datasete boli príslušné informácie dostupné. Pipeline preto bola upravená tak, aby sa pri konštrukcii scenárov zachoval modelový text použitý na tréning, ale zároveň sa doplnili chýbajúce surové polia potrebné pre feature extraction.

Konkrétne pri scenároch typu M sa surové tréningové polia obohacovali zo zdieľaného súboru \texttt{shared/train\_full.csv} podľa identifikátorov \texttt{row\_id} a \texttt{synthetic\_parent\_row\_id}. Takýto backfill nemenil text, na ktorom sa model učil, ale zvyšoval pokrytie technických príznakov používaných v heuristickej aj modelovej vrstve. Tým sa podarilo korektne zachytiť napríklad indikátory \texttt{display\_name\_spoof\_flag}, \texttt{message\_id\_domain\_mismatch}, \texttt{external\_domain\_url\_count}, \texttt{sender\_url\_domain\_mismatch\_ratio}, \texttt{obfuscated\_url\_count}, \texttt{mismatched\_anchor\_count} a \texttt{punycode\_url\_count}.
...
Na elimináciu týchto problémov bola pipeline postupne rozšírená o viacero obranných mechanizmov. Najprv bola zavedená silnejšia textová normalizácia, následne source-aware spracovanie, explicitné limitovanie dominantných zdrojov, tokenové maskovanie výrazov prezrádzajúcich pôvod datasetu a napokon aj template-aware splitovanie a template-aware regulácia tréningových scenárov. V záverečnej iterácii pribudla aj SHAP-guided kontrola feature priestoru, ktorá umožnila rozlíšiť, ktoré ručne definované príznaky prinášajú reálny prínos a ktoré sú v danom scenári prakticky neaktívne.
...
Jednou z najdôležitejších metodických zmien bolo zavedenie zdieľaných splitov, ktoré sú spoločné pre všetky scenáre. Vďaka tomu bolo možné férovo porovnávať jednotlivé tréningové stratégie, keďže rozdiely vo výsledkoch nevznikali z iného náhodného rozdelenia dát, ale priamo zo spôsobu konštrukcie tréningovej množiny. Finálna verzia prípravného skriptu preto pri vytváraní scenára už negenerovala nové splity, ale explicitne \textbf{reused existujúce train/val/test rozdelenie} a menila len pravidlá konštrukcie tréningovej časti.
...
V záverečnej iterácii boli doplnené aj \textbf{semantické subclass feature}, ktoré explicitne reprezentujú typ výzvy v správe. Pre phishing-oriented obsah išlo najmä o príznaky \texttt{cta\_login\_flag}, \texttt{cta\_verify\_flag}, \texttt{account\_security\_phrase\_count}, pre finančný podvod o \texttt{invoice\_reference\_flag}, \texttt{remittance\_reference\_flag}, \texttt{cta\_payment\_flag}, \texttt{due\_date\_pattern\_flag} a pre marketingovo orientovaný spam o \texttt{spam\_reward\_flag}, \texttt{spam\_unsubscribe\_context\_flag} a \texttt{spam\_marketing\_phrase\_count}. Tieto príznaky boli navrhnuté ako deterministická extrakčná vrstva nad textom, nie ako ručné dopĺňanie labelov.
...
V neskoršej fáze práce bol feature priestor auditovaný pomocou SHAP analýzy. Tá ukázala, že časť ručne navrhnutých príznakov neprináša v generalistickom scenári M reálny zisk, pretože je v tréningových dátach prakticky neaktívna alebo redundatná. Finálny general/offline model preto nevyužíva celý pôvodný zoznam numerických príznakov, ale iba SHAP-guided subset 11 najužitočnejších feature. Medzi najstabilnejšie zostávajúce príznaky patrili najmä \texttt{subject\_len}, \texttt{uppercase\_ratio}, \texttt{digit\_ratio}, \texttt{sender\_receiver\_domain\_mismatch}, \texttt{financial\_keyword\_count}, \texttt{exclamation\_count} a ďalšie frekvenčné alebo štylistické indikátory.

Samostatný SHAP audit bol následne vykonaný aj nad novými semantickými feature v kampanových evaluáciách. Ako najsilnejšie sa ukázali najmä \texttt{cta\_login\_flag}, \texttt{remittance\_reference\_flag}, \texttt{cta\_verify\_flag}, \texttt{invoice\_reference\_flag} a \texttt{due\_date\_pattern\_flag}. Naopak nie všetky novo pridané spam feature vykazovali globálne silný SHAP signál na offline trénovacích dátach, čo zodpovedá ich účelu: ide skôr o jemnú subclass špecializáciu pre campaign benchmarky než o všeobecne dominantné offline indikátory.
...
Okrem samotného lineárneho klasifikátora boli neskôr testované aj rôzne rozhodovacie hlavy nad rovnakou textovo-feature reprezentáciou. Základnou voľbou bol multiclass single-stage model, ktorý priamo predikuje jednu zo štyroch cieľových tried. Pre špecializované campaign benchmarky bol následne doplnený aj variant s \textit{one-vs-rest} subclass hlavou, ktorý po binárnom oddelení triedy \texttt{legit} samostatne skóruje triedy \texttt{phishing}, \texttt{spam} a \texttt{financial\_fraud}. Tento OVR prístup sa ukázal ako užitočný najmä v situáciách, kde pôvodný multiclass model kolaboval viaceré škodlivé triedy do jednej dominantnej kategórie. V poslednej iterácii bol OVR tréning ďalej posilnený pair-specific hard negatives pre hranice \texttt{phishing-spam} a \texttt{financial\_fraud-spam}, čo ešte zlepšilo subclass separáciu.
...
Porovnanie jednotlivých vstupných reprezentácií pre tuned single-stage model scenára M je znázornené na obrázku~\ref{fig:input_comparison_final_m}. Z grafu je zrejmé, že varianty \texttt{header\_only} a \texttt{subject\_only} dosahujú výrazne nižšie hodnoty macro F1, čo naznačuje, že tieto časti emailu samy osebe neposkytujú dostatočne robustný signál pre klasifikáciu.
...
\begin{figure}[H]
    ...
    \caption{Porovnanie vstupných reprezentácií pre tuned single-stage model scenára M (hybrid\_logreg).}
    \label{fig:input_comparison_final_m}
\end{figure}

\textbf{Single-stage, OVR subclass head a two-stage prístup} 

V experimentálnej časti boli porovnané tri základné prístupy ku klasifikácii. Prvým bol single-stage model, ktorý priamo priraďuje email do jednej z cieľových tried. Druhým bol OVR subclass head, pri ktorom sa po oddelení legitímnych správ samostatne odhadujú skóre pre jednotlivé podozrivé triedy. Tretím bol two-stage prístup, pri ktorom prvá fáza rozhoduje medzi benign a suspicious správami a druhá fáza následne rozlišuje konkrétnu kategóriu medzi podozrivými správami.

Plný two-stage prístup je intuitívne atraktívny, pretože kopíruje viacúrovňové rozhodovanie známe z bezpečnostných systémov. V našich experimentoch však nepriniesol konzistentné zlepšenie oproti single-stage klasifikácii. Naopak, vo viacerých benchmarkoch boli single-stage modely stabilnejšie a jednoduchšie interpretovateľné. OVR subclass hlava sa ukázala ako zaujímavý kompromis: na všeobecných offline splitoch neprekonala tuned single-stage model, ale na špecializovaných header/auth a URL campaign benchmarkoch priniesla výrazné zlepšenie multiclass diskriminácie.

Z týchto dôvodov bol single-stage prístup zvolený ako hlavný general/offline smer a two-stage bol ponechaný ako porovnávacia vetva vhodná do experimentálnej a diskusnej časti práce. OVR subclass hlava bola nakoniec zaradená ako špecializovaný campaign-oriented variant nad rovnakým scenárom M.
...
Na základe realizovaných experimentov sa ako najrelevantnejší ukázal scenár M a jeho ďalšie odvodené varianty. Pôvodný scenár M poskytol metodicky čistý anti-template základ, ku ktorému boli neskôr pridané dve rôzne optimalizačné vetvy.

Prvou vetvou bol SHAP-guided tuned single-stage model, ktorý zachováva metodickú čistotu scenára M, ale redukuje feature priestor na najstabilnejší subset a upravuje tréningové váhy. Tento variant sa ukázal ako najsilnejší z pohľadu všeobecnej robustnosti na shared offline splitoch a preto bol zvolený ako primárny general/offline model:
\begin{itemize}
    \item \textbf{Primárny general/offline model:} \\
    \textit{scenario\_m\_anti\_template\_feature\_regularized + hybrid\_logreg + text\_plus\_features + SHAP-guided feature subset}
\end{itemize}

Druhou vetvou bol campaign-specialized variant, ktorý kombinuje ne-leaky campaign-style adaptačný tréning, zúženie spam triedy, semantické CTA/payment/spam feature a OVR subclass hlavu s pair-specific hard negatives. Tento variant sa ukázal ako najsilnejší na kampaniach z Experimentov 1 až 3:
\begin{itemize}
    \item \textbf{Špecializovaný campaign model:} \\
    \textit{scenario\_m\_anti\_template\_feature\_regularized + hybrid\_logreg + text\_plus\_features + adaptation + semantic CTA/payment/spam features + spam narrowing + OVR + pair-hardneg}
\end{itemize}

Pri tomto variante bol vykonaný aj samostatný leakage audit medzi adaptačným train setom a seed benchmarkmi z Experimentov 2 a 3. Audit ukázal nulový presný overlap na úrovni subjectov aj textov. Zároveň sa objavili iba blízke, nie však identické subjectové parafrázy (najvyššia podobnosť približne 0.80--0.86), čo zodpovedá rovnakej rodine úloh, nie priamemu data leakage.
...
Takto definovaný výber lepšie odráža reálny tréningový vývoj než pôvodné porovnanie L verzus M v izolovanej podobe a zároveň vysvetľuje, prečo sa v neskorších kapitolách objavujú dva rôzne „finálne“ modely podľa typu evaluácie.
...
\begin{table}[H]
\centering
\caption{Porovnanie finálnych kandidátov na robustnostných splitoch (macro F1)}
...
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{test\_hard\_source} & \textbf{test\_hard\_cluster} & \textbf{test\_deployment} \\
\midrule
\textbf{Referenčný baseline:} L+SGD & 0.6941 & 0.8196 & 0.8763 \\
\textbf{Primárny general/offline model:} tuned single-stage & \textbf{0.8744} & \textbf{0.8638} & \textbf{0.9076} \\
\textbf{Špecializovaný campaign model:} semantic OVR & 0.7011 & 0.7512 & 0.7865 \\
\bottomrule
\end{tabular}

\end{table}

Výsledky potvrdzujú, že scenár M je výrazne robustnejší než referenčný anti-source baseline L. Zároveň je zrejmé, že SHAP-guided tuned single-stage konfigurácia \textit{scenario\_m\_anti\_template\_feature\_regularized + hybrid\_logreg} dosahuje najlepší celkový kompromis na náročných offline testoch. Semantic OVR variant síce predstavuje finálny nasadzovaný model nástroja, no na shared offline splitoch zaostáva za tuned single-stage baselineom, čo zodpovedá jeho špecializácii na campaign-like dáta.
...
Ako finálny nasadzovaný model bol zvolený semantický OVR variant nad scenárom
\texttt{scenario\_m\_anti\_template\_feature\_regularized + hybrid\_logreg + text\_plus\_features},
doplnený o campaign-style adaptáciu, semantické CTA/payment/spam feature,
zúženie spam triedy a pair-specific hard negatives.

Pre objektívne zhodnotenie bol model analyzovaný aj na nezávislom testovacom splite \texttt{test\_deployment}, ktorý lepšie reprezentuje podmienky širšieho reálneho nasadenia než experimentálne dáta z kampane.
...
Trieda \texttt{legit} je klasifikovaná veľmi spoľahlivo, čo dokumentuje vysoký počet správne klasifikovaných vzoriek (14233) a relatívne nízky počet chýb smerom k ostatným triedam (najmä 566 do \texttt{phishing} a 184 do \texttt{spam}). To naznačuje, že model dokáže robustne odlišovať bežnú komunikáciu od škodlivého obsahu, hoci v porovnaní s tuned single-stage baseline je jeho offline rozhodovanie agresívnejšie.

Trieda \texttt{phishing} dosahuje taktiež veľmi dobré výsledky (2595 správne klasifikovaných vzoriek), pričom chyby sú rozdelené medzi \texttt{financial\_fraud} (72 prípadov) a \texttt{legit} (40 prípadov). Tento vzor chýb odráža prirodzené prekrytie medzi manipulatívnym sociálnym inžinierstvom a finančnými podvodmi.

Trieda \texttt{financial\_fraud} je klasifikovaná s vysokým recallom (338 správnych klasifikácií), pričom časť chýb smeruje do tried \texttt{spam} (14 prípadov) a \texttt{phishing} (10 prípadov). Tento jav je očakávaný, keďže finančné podvody často využívajú podobné jazykové vzory ako iné sociálno-inžinierske útoky.

Najproblematickejšou triedou ostáva \texttt{spam}. Aj keď model správne klasifikoval 587 vzoriek, dochádza k zámene najmä s \texttt{legit} (192 prípadov). To naznačuje, že spam-semantic špecializácia, ktorá výrazne pomáha na campaign benchmarkoch, sa na širšom deployment datasete stretáva s väčšou rozmanitosťou legitímnych marketingových alebo oznamovacích správ.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{final_model_confusion_matrix_test_deployment.pdf}
    \caption{Konfúzna matica finálneho semantického OVR modelu na splite test\_deployment}
    \label{fig:confusion_matrix}
\end{figure}

Z pohľadu praktického nasadenia model vykazuje veľmi dobrú schopnosť rozlišovať medzi legitímnou a škodlivou komunikáciou. Najväčšie chyby vznikajú pri jemnej diferenciácii medzi spamom a legitímnym marketingovým alebo oznamovacím obsahom, čo je typické aj pre reálne bezpečnostné systémy.

Dôležitým pozorovaním je rozdiel medzi experimentálnymi dátami (Experimenty 1 až 3) a deployment splitom. Kým na campaign benchmarkoch semantický OVR model dosahuje veľmi vysoké multiclass výsledky, na \texttt{test\_deployment} sa objavuje realistickejší trade-off medzi precíznou subclass diferenciáciou a všeobecnou robustnosťou. Tento rozdiel neznamená priamy leakage, ale skôr to, že campaign-style adaptácia posúva model smerom k praktickému emailovému prostrediu, nie k maximálnej robustnosti na širšej offline distribúcii.

Z pohľadu metodickej čistoty aj praktického použitia preto v práci rozlišujeme dve referencie: finálny nasadzovaný model nástroja je semantický OVR variant, zatiaľ čo tuned single-stage model ostáva dôležitým offline baselineom a porovnávacím bodom voči scenáru L.
...
\begin{figure}[H]
    ...
    \caption{Porovnanie heuristického prístupu, tuned single-stage modelu a finálneho semantického OVR variantu na kampani z Experimentu 1.}
    \label{fig:exp1_model_comparison}
\end{figure}

Z grafu je zrejmé, že heuristický analyzátor dosahuje vysoký recall, avšak výrazne zaostáva v metrikách balanced accuracy a MCC. Tuned single-stage model aj finálny semantický OVR variant poskytujú stabilnejšiu separáciu tried a lepší kompromis medzi precision a recall. Semantický OVR model síce na zmiešanej kampani nedosiahol najvyššie binárne F1, ale výrazne zlepšil multiclass správanie.
...
\begin{figure}[H]
    ...
    \caption{Porovnanie multiclass metrík heuristiky, tuned single-stage modelu a finálneho semantického OVR variantu na dátach Experimentu 1.}
    \label{fig:exp1_multiclass_comparison}
\end{figure}

Výsledky ukazujú, že samotná heuristika dosahuje výrazne nižší multiclass výkon než modelové prístupy. Na zmiešanom sender-clean benchmarku sa po doplnení semantických CTA/payment/spam feature a pair-specific hard negatives ukázal ako najsilnejší multiclass variant semantický OVR model (\texttt{mc\_f1\_macro = 0.7989}), ktorý výrazne prekonal tuned single-stage baseline (\texttt{0.6213}). Tento výsledok ukazuje, že vhodne navrhnuté semantické feature dokážu znížiť aj predchádzajúce problémy so spam triedou.
...
\begin{figure}[H]
    ...
    \caption{Konfúzna matica finálneho semantického OVR modelu na dátach Experimentu 1.}
    \label{fig:exp1_confusion_matrix}
\end{figure}

Z konfúznej matice je možné pozorovať, že finálny semantický OVR model výrazne zlepšil správanie voči triede \texttt{spam} a zároveň zachoval vysoký recall pre \texttt{phishing} a \texttt{financial\_fraud}. Najväčšie chyby vznikajú pri triede \texttt{legit}, ktorej časť správ je stále posúvaná smerom k podozrivým triedam.
...
Z pohľadu výsledkov sa ako najsilnejší variant pre samotný Experiment 1 ukázal semantický OVR model so scenárom M, campaign-style adaptáciou, spam semantic boostom a pair-specific hard negatives. Pri vyhodnotení na 250 doručených správach dosiahol \texttt{bin\_f1 = 0.9614} a \texttt{mc\_f1\_macro = 0.7989}. Oproti tuned single-stage baseline tak síce mierne klesol binárny výkon (\texttt{0.9727} $\rightarrow$ \texttt{0.9614}), ale multiclass macro F1 sa výrazne zvýšilo (\texttt{0.6213} $\rightarrow$ \texttt{0.7989}).

Experiment 1 preto ukázal dôležitý trade-off: tuned single-stage model ostáva vhodnejší ako všeobecný robustný model, no pri samotnej zmiešanej kampani dokáže semantický OVR variant podstatne lepšie rozlišovať medzi triedami \texttt{phishing}, \texttt{spam} a \texttt{financial\_fraud}. Heuristický analyzátor ostáva vhodným doplnkom pre prvotný screening, avšak finálne rozhodovanie je výhodnejšie ponechať na modelovej vrstve.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\linewidth]{exp2_header_auth_indicator_contribution.pdf}
    \caption{Relatívna sila hlavných hlavičkových a autentifikačných indikátorov v heuristickom skórovaní Experimentu 2. Najväčší pozitívny príspevok nesú \texttt{SPF fail} a \texttt{DMARC fail}, zatiaľ čo \texttt{trusted\_domain\_only} funguje ako guardrail proti false positive prípadom. Indikátor \texttt{Message-ID} mismatch je v tejto verzii analyzátora sledovaný, ale neprispieva priamo do heuristického skóre.}
    \label{fig:exp2_indicator_contribution}
\end{figure}

Obrázok~\ref{fig:exp2_indicator_contribution} pomáha odlíšiť dva aspekty Experimentu 2: po prvé, či analyzátor dokáže indikátor korektne detegovať, a po druhé, akú silu má daný indikátor v samotnom heuristickom rozhodovaní. Z pohľadu skórovania sú najsilnejšie autentifikačné zlyhania \texttt{SPF} a \texttt{DMARC}, nasledované nesúladmi identít odosielateľa a zlyhaním \texttt{DKIM}. Naopak \texttt{trusted\_domain\_only} znižuje skóre pri dôveryhodnej komunikácii a slúži ako ochrana proti falošným poplachom.
...
Pri neskoršom vyhodnotení finálneho semantického OVR variantu nebolo potrebné kampaň seedovať nanovo. Použité boli tie isté už doručené a uložené \texttt{.eml} správy, nad ktorými bola znovu spustená aktualizovaná pipeline extrakcie príznakov a nový modelový variant. Vďaka tomu ostáva porovnanie medzi baseline a finálnym modelom metodicky čisté a \textit{apples-to-apples}.

Zároveň bol nad adaptačným train setom vykonaný leakage audit. Ten ukázal nulový presný overlap subjectov aj textov medzi adaptation dátami a seed benchmarkom Experimentu 2. Objavili sa iba blízke subjectové parafrázy (približne \texttt{0.80--0.86} podobnosti), ktoré zodpovedajú rovnakej rodine úloh, nie priamemu preniknutiu eval vzoriek do tréningu.
...
\texttt{ml\_only}               & \textbf{0.96} & \textbf{0.90} & \textbf{0.9524} & \textbf{1.0000} & \textbf{0.9756} \\
\texttt{heuristic\_plus\_ml}    & 0.91 & 0.775 & 0.8989 & 1.0000 & 0.9467 \\
\bottomrule
\end{tabular}
\end{table}
...
Najlepší výkon dosiahol prístup \texttt{ml\_only}, ktorý v tomto experimente používa campaign-style adaptačný tréning, zúženie spam triedy, semantické CTA/payment/spam feature a \textit{one-vs-rest} subclass hlavu. Tento variant poskytuje výrazne lepší kompromis medzi precision a recall a zároveň dosahuje najvyššiu hodnotu F1 skóre (\texttt{0.9756}). Kombinovaný prístup \texttt{heuristic\_plus\_ml} ostáva silný, no už nedosahuje výkon samostatného adaptovaného modelu.

Z doplnkového pohľadu boli analyzované aj multiclass výsledky, ktoré však nie sú primárnym cieľom tohto experimentu. V tomto režime heuristika dosahuje nízke hodnoty (napr. \texttt{mc\_f1\_macro = 0.13}), čo je očakávané, keďže nie je navrhnutá na jemné rozlišovanie medzi triedami \texttt{phishing}, \texttt{spam} a \texttt{financial\_fraud}. Finálny semantický OVR model dosahuje v multiclass nastavení výrazne lepší výsledok než pôvodný tuned single-stage variant aj než prvá verzia adapted+OVR (\texttt{mc\_f1\_macro = 0.9557} vs. \texttt{0.3262} a \texttt{0.5865}). To ukazuje, že pri header/auth kampani bol hlavný problém v nesúlade medzi tréningovou distribúciou a campaign-style vstupmi a v chýbajúcej semantickej subclass reprezentácii, nie v samotnej kapacite modelu.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\linewidth]{exp2_per_class_baseline_vs_adapted_ovr.pdf}
    \caption{Porovnanie per-class F1 v Experimente 2 pred a po zavedení semantického OVR variantu s campaign-style adaptáciou a zúžením spam triedy.}
    \label{fig:exp2_per_class_gain}
\end{figure}

Obrázok~\ref{fig:exp2_per_class_gain} ukazuje, že najväčší prínos nového variantu sa objavil pri triedach \texttt{phishing} a \texttt{financial\_fraud}, ktoré mali v baseline prakticky nulové F1. Po doplnení semantických feature sa však výrazne zlepšila aj trieda \texttt{spam}, takže model už neodstraňuje kolaps do \texttt{spam} len váhami, ale skutočne lepšie rozumie typu výzvy v správe.
...
\begin{itemize}
    ...
    \item campaign-style adaptačný tréning bez leakage výrazne zlepšuje prenositeľnosť modelu na realistickú hlavičkovú kampaň,
    \item zúženie spam triedy, pair-specific OVR hard negatives a semantické CTA/payment/spam feature výrazne znižujú kolaps tried \texttt{phishing} a \texttt{financial\_fraud} do \texttt{spam}.
\end{itemize}

Z pohľadu návrhu systému sa preto ako vhodné riešenie javí architektúra, v ktorej heuristický analyzátor slúži ako doplnkový alebo signalizačný modul (napr. pre vysvetliteľnosť alebo zvýraznenie rizikových príznakov), zatiaľ čo finálne rozhodovanie o kategórii správy je ponechané na adaptovaný model strojového učenia. V tomto experimente sa ukázalo, že samotná úprava rozhodovacieho headu a zladenie tréningovej distribúcie s campaign-style vstupmi prinášajú väčší efekt než jednoduché pravidlové kombinovanie heuristiky a ML modelu.
...
Heuristický analyzátor spoľahlivo deteguje explicitné bezpečnostné indikátory a funguje ako prvá vrstva rozhodovania (legit vs suspicious). Najlepšie výsledky však v Track B dosiahol finálny semantický OVR model s adaptačným tréningom, zúžením spam triedy a pair-specific hard negatives, ktorý dosiahol \texttt{bin\_f1 = 0.9756} a \texttt{mc\_f1\_macro = 0.9557}.

Z metodického hľadiska sa preto ako najvhodnejší prístup javí využitie heuristiky ako doplnkového a vysvetľujúceho modulu, zatiaľ čo hlavné rozhodovanie o kategórii správy by malo byť ponechané na adaptovaný model strojového učenia.
...
Na realistickej kampani boli porovnané tri prístupy: \texttt{heuristic\_only}, \texttt{ml\_only} a \texttt{heuristic\_plus\_ml}. Rovnako ako v Experimente 2, aj tu bol finálny semantický OVR variant vyhodnotený na tých istých už uložených campaign EML bez nového seedingu kampane; zmenila sa len extrakcia príznakov a modelová vrstva.

Leakage audit nad adaptačným train setom ukázal nulový presný overlap subjectov aj textov so seed benchmarkom Experimentu 3. Objavili sa iba blízke subjectové parafrázy (približne \texttt{0.80--0.84} podobnosti), čo je konzistentné s používaním odlišných šablón v rovnakej úlohovej rodine, nie s priamym preniknutím eval textov do tréningu.
...
\hline
heuristic\_only & 0.81 & 0.8081 & 1.00 & 0.8939 \\
ml\_only & 0.98 & 0.9756 & 1.0000 & 0.9877 \\
heuristic\_plus\_ml & 0.91 & 0.8989 & 1.00 & 0.9467 \\
\hline
\end{tabular}
\end{table}

Heuristický analyzátor dosahuje maximálny recall, čo znamená, že zachytáva všetky podozrivé správy. Na druhej strane však nedokáže efektívne odlišovať legitímne správy, čo sa prejavuje vyšším počtom false positive.

Po zavedení campaign-style adaptačného tréningu, semantických CTA/payment feature, pair-specific OVR hard negatives a ponechaní spam špecializácie bez dodatočného spam boostu dosiahol samotný model strojového učenia na URL-heavy kampani najlepší binárny výkon (\texttt{F1 = 0.9877}). Kombinovaný prístup \texttt{heuristic\_plus\_ml} už v tomto nastavení zaostáva.
...
Model strojového učenia po adaptácii vykazuje vyváženejšie správanie a jeho chybovosť už nie je tak silno viazaná na jednoduchý kolaps všetkých podozrivých správ do triedy \texttt{spam}. V multiclass režime dosiahol \texttt{mc\_f1\_macro = 0.9799}, čo predstavuje výrazné zlepšenie oproti predchádzajúcemu tuned single-stage variantu (\texttt{0.3003}) aj oproti staršiemu adapted+OVR variantu (\texttt{0.6052}).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\linewidth]{exp3_per_class_baseline_vs_adapted_ovr.pdf}
    \caption{Porovnanie per-class F1 v Experimente 3 pred a po zavedení finálneho semantického OVR variantu.}
    \label{fig:exp3_per_class_gain}
\end{figure}

Na obrázku~\ref{fig:exp3_per_class_gain} je viditeľné, že URL-heavy benchmark profitoval zo zlepšenia všetkých troch podozrivých tried. Najvýraznejší posun nastal pri \texttt{financial\_fraud} a \texttt{spam}, pričom aj trieda \texttt{phishing} už nekolabuje do \texttt{spam} ako v baseline variante.
...
Experiment 3 ukázal, že URL-heavy benchmark výrazne profituje z priblíženia tréningovej distribúcie ku campaign-style vstupom a z doplnenia semantických CTA/payment feature. Finálny semantický OVR model dokázal výrazne zlepšiť nielen binárny výkon, ale aj jemné multiclass rozlíšenie medzi \texttt{phishing}, \texttt{spam} a \texttt{financial\_fraud}. URL heuristiky preto ostávajú cenným doplnkovým signálom, no ich najlepší efekt sa dosiahol až po špecializácii tréningu, feature priestoru a rozhodovacieho headu modelu.
...
\caption{Porovnanie tuned single-stage a finálneho semantického OVR variantu na hlavných offline splitoch v Experimente 4 (Track A). Graf ukazuje, že semantický OVR model pomáha na campaign benchmarkoch, ale na shared offline splitoch zaostáva za tuned single-stage modelom.}
\label{fig:exp4_trackA_deployment}
\end{figure}

Výsledky ukazujú, že tuned single-stage model ostáva na shared offline splitoch jednoznačne silnejší než finálny semantický OVR variant. Variant \texttt{scenario\_m\_anti\_template\_feature\_regularized + hybrid\_logreg} dosiahol na \texttt{test\_deployment} \texttt{macro F1 = 0.9076} a súčasne výrazne zlepšil výsledok na \texttt{test\_hard\_source} na \texttt{0.8744}. Dodatočne testovaný semantický OVR model síce výrazne pomohol na Experimentoch 1 až 3, no na offline splitoch klesol na \texttt{0.7917} (\texttt{test\_iid}), \texttt{0.7011} (\texttt{test\_hard\_source}), \texttt{0.7512} (\texttt{test\_hard\_cluster}) a \texttt{0.7865} (\texttt{test\_deployment}).
...
aj v kampanovom prostredí. Na zmiešanej kampani z Experimentu 1
dosiahol tuned single-stage variant \texttt{bin\_f1 = 0.9727}
a \texttt{mc\_f1\_macro = 0.6213}. Finálny semantický OVR variant
na tej istej kampani dosiahol \texttt{bin\_f1 = 0.9614}, ale
multiclass výkon zvýšil až na \texttt{0.7989}. Táto časť preto
ukazuje, že Exp1 už nie je najlepšie reprezentovaný iba tuned modelom:
pre obsahovo zmiešanú kampaň sa ako najsilnejší multiclass variant
ukázal semantický OVR model, zatiaľ čo tuned single-stage ostáva
silnejší ako general/offline baseline.
...
Na kampani z Experimentu~2 \ref{epx2} dosiahol finálny semantický
OVR variant hodnoty
\begin{itemize}
    \item \texttt{mc\_accuracy = 0.96},
    \item \texttt{mc\_balanced\_accuracy = 0.95},
    \item \texttt{mc\_f1\_macro = 0.9557}.
\end{itemize}

Na kampani z Experimentu~3 \ref{epx3} dosiahol
\begin{itemize}
    \item \texttt{mc\_accuracy = 0.98},
    \item \texttt{mc\_balanced\_accuracy = 0.975},
    \item \texttt{mc\_f1\_macro = 0.9799}.
\end{itemize}


Pre kontext je vhodné uviesť výkon v Experimente~1\ref{epx1},
kde finálny semantický OVR variant dosiahol \texttt{mc\_f1\_macro = 0.7989}.
To ukazuje, že po doplnení semantických spam feature sa campaign-specialized
vetva stala silnou nielen pre špecializované benchmarky z Experimentov~2 a~3,
ale aj pre zmiešaný sender-clean scenár.

V porovnaní s binárnym hodnotením ostáva multiclass úloha náročnejšia,
avšak finálny semantický OVR variant výrazne znížil pôvodný kolaps tried
\texttt{phishing} a \texttt{financial\_fraud} do \texttt{spam}.
Výsledky tak ukazujú, že špecializácia tréningu, feature priestoru a
rozhodovacieho headu môže výrazne zlepšiť subclass diskrimináciu, no za cenu
horšej všeobecnej offline robustnosti.
...
Na základe kombinácie výsledkov zo všetkých splitov a kampaní bol ako
\textbf{finálny nasadzovaný model nástroja} zvolený semantický OVR variant,
ktorý kombinuje campaign-style adaptačný tréning bez leakage,
semantické CTA/payment/spam feature, zúženie spam triedy,
\textit{one-vs-rest} subclass hlavu a pair-specific hard negatives.
Tento variant dosiahol na Exp1 \texttt{bin\_f1 = 0.9614}, \texttt{mc\_f1\_macro = 0.7989},
na Exp2 \texttt{bin\_f1 = 0.9756}, \texttt{mc\_f1\_macro = 0.9557} a na Exp3
\texttt{bin\_f1 = 0.9877}, \texttt{mc\_f1\_macro = 0.9799}. Z pohľadu realistických
kampaní teda poskytol najsilnejšie a najkonzistentnejšie výsledky.

Tuned single-stage model ostáva dôležitým \textbf{offline referenčným baselineom},
pretože na shared offline splitoch dosahuje vyššie hodnoty
(\texttt{0.9105}/\texttt{0.8744}/\texttt{0.8638}/\texttt{0.9076}), zatiaľ čo
finálny semantický OVR model na tých istých splitoch dosiahol
\texttt{0.7917}/\texttt{0.7011}/\texttt{0.7512}/\texttt{0.7865}. Napriek tomu bol pre účely
praktického nasadenia v nástroji uprednostnený semantický OVR variant,
keďže cieľom systému je primárne kvalitné rozhodovanie v campaign-like emailovom prostredí.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{exp4_general_vs_campaign_tradeoff.pdf}
    \caption{Trade-off medzi tuned single-stage a finálnym semantickým OVR variantom na zmiešanej kampani Experimentu 1, špecializovaných kampaniach Experimentov 2 a 3 a offline splite \texttt{test\_deployment}.}
    \label{fig:exp4_tradeoff}
\end{figure}

Obrázok~\ref{fig:exp4_tradeoff} sumarizuje hlavný záver Experimentu 4: tuned single-stage model ostáva silnejší na offline benchmarkoch, no finálny semantický OVR variant bol vybraný ako prakticky najvhodnejší model pre nástroj, pretože výrazne prekonáva baseline na kampaniach z Experimentov 1 až 3.
...
Výsledky zároveň ukazujú, že kombinácia vhodného tréningového
scenára, rozhodovacieho headu, dátovej špecializácie a semantických
subclass feature je kľúčová pre dosiahnutie optimálneho výkonu.
Finálny semantický OVR variant bol preto zvolený ako hlavný model
detekčného nástroja, zatiaľ čo tuned single-stage model zostáva
referenčným offline baselineom pre robustnostné porovnania.
...
Ako reprezentant modelového prístupu bol použitý finálny semantický OVR model zo scenára
\texttt{scenario\_m\_anti\_template\_feature\_regularized} s klasifikátorom
\texttt{hybrid\_logreg} a vstupným režimom \texttt{text\_plus\_features}.
Model využíva campaign-style adaptačný tréning bez leakage, semantické CTA/payment/spam feature,
zúženie spam triedy a \textit{one-vs-rest} subclass hlavu s pair-specific hard negatives.
Tento variant bol zvolený na základe výsledkov Experimentu 4 \ref{exp4}, kde sa ukázal
ako najpraktickejší model pre campaign-like prostredie, hoci tuned single-stage ostal silnejší
na čistých offline splitoch.
...
\midrule
\texttt{heuristic\_only}         & 0.76 & 0.75 & 0.69 & 0.52 \\
\texttt{ml\_only}                & \textbf{0.89} & \textbf{0.89} & \textbf{0.88} & \textbf{0.78} \\
\texttt{hybrid\_a\_conservative} & 0.88 & 0.88 & 0.87 & 0.76 \\
\texttt{hybrid\_b\_gated}        & 0.76 & 0.75 & 0.69 & 0.52 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
avšak neprináša výrazné zlepšenie. Oproti staršiemu tuned single-stage benchmarku
sa síce globálne binárne F1 mierne znížilo, no je to dôsledok toho, že finálny
semantický OVR model uprednostňuje presnejšie multiclass rozhodovanie na campaign-like dátach.
...
\midrule
controlled benchmark   & \textbf{0.736} & 0.652 & 0.695 & \textbf{0.736} \\
realistic campaign     & 0.827  & \textbf{0.967}  & 0.941  & 0.827 \\
offline generalization & 0.036  & \textbf{0.822}  & \textbf{0.822}  & 0.036 \\
\bottomrule
\end{tabular}%
}
...
V kontrolovanom benchmarku ostávajú heuristické pravidlá veľmi silné,
čo naznačuje, že v prostredí s presne definovanými indikátormi majú
stále vysokú hodnotu ako validácia implementovaných pravidiel.
...
a nízku schopnosť adaptácie na nové dáta. Finálny semantický OVR model
si aj v tejto vetve zachováva výrazne vyšší výkon než heuristika,
hoci za tuned single-stage offline baselineom stále zaostáva.
...
ml\_only & 0.864 \\
hybrid\_a\_conservative & 0.864 \\
hybrid\_b\_gated & 0.679 \\
\hline
\end{tabular}
...
čo vedie k nízkemu výkonu. Finálny semantický OVR model a hybrid A dosahujú výrazne
lepšie výsledky, keďže využívajú komplexnejšie reprezentácie dát a explicitnú subclass
semantiku.
...
by malo byť založené na semantickom OVR modeli.
...
\hline
Náš nástroj vs truth & 0.88 & 0.941 & 0.889 & 0.914 \\
...
Zároveň bola pozorovaná zhoda medzi nástrojmi na úrovni 0.88.

Výsledky ukazujú zásadný rozdiel v správaní oboch prístupov:

\begin{itemize}
    \item náš nástroj je \textbf{stále konzervatívnejší}, no po nasadení semantického OVR modelu sa zlepšil recall pri zachovaní vysokej precision (0.941),
...
Na druhej strane náš systém po doplnení finálneho semantického OVR modelu ponúka jednoduchšie,
konzistentné a automatizované riešenie,
ktoré je vhodnejšie pre bežných používateľov alebo
automatizované spracovanie e-mailov. V decision-level porovnaní sa náš výsledok
výrazne priblížil PhishToolu (F1 \texttt{0.914} vs. \texttt{0.947}).
...
detekčného systému, ktorá používa semantický OVR variant ako hlavný
nasadzovaný model a tuned single-stage ponecháva ako offline referenčný
baseline. Toto rozdelenie umožňuje zachovať metodickú transparentnosť a
zároveň pracovať s modelom, ktorý podáva najlepšie výsledky na praktických
campaign benchmarkoch.

\paragraph{Zvolená konfigurácia}

\begin{itemize}
    \item \textbf{Primárny nasadzovaný model:} campaign-style adaptačný tréning bez leakage, semantické CTA/payment/spam feature, zúženie spam triedy, \textit{one-vs-rest} subclass hlava a pair-specific hard negatives nad scenárom \texttt{scenario\_m\_anti\_template\_feature\_regularized + hybrid\_logreg}.
    \item \textbf{Referenčný offline baseline:} \texttt{scenario\_m\_anti\_template\_feature\_regularized + hybrid\_logreg} v režime \texttt{text\_plus\_features} so SHAP-guided feature subsetom.
    \item \textbf{Rozhodovanie:} finálne rozhodnutie realizuje ML model, pričom heuristika slúži ako doplnková vrstva pre risk score a explainability.
    \item \textbf{Nasadenie:}
    \begin{itemize}
        \item pre praktické nasadenie v campaign-like emailovom prostredí je preferovaný semantický OVR model,
        \item tuned single-stage model ostáva referenčným baselineom pre offline robustnostné porovnania.
    \end{itemize}
...
\begin{itemize}
    ...
    pričom výber konkrétneho variantu musí rešpektovať rozdiel medzi generalistickým
    a campaign-specialized nastavením.
    
    \item \textbf{Experiment 1, 2 a 3:}
    heuristické indikátory sú veľmi presné na detekciu konkrétnych znakov,
    no najväčšie zlepšenie priniesla až kombinácia campaign-style adaptačného tréningu,
    semantických subclass feature, zúženia spam triedy a OVR subclass hlavy.
...
a heuristika slúži ako doplnkový zdroj informácií. Zároveň sa ukázalo,
že v praxi je vhodné rozlišovať medzi general/offline variantom modelu
a špecializovaným campaign-oriented variantom pre úzko zamerané benchmarky.
